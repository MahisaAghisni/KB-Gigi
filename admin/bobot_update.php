...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE bobot SET kd_gejala=%s, kd_penyakit=%s, bobot=%s WHERE id_bobot=%s",
...
$colname_rsBobotUpdate = "-1";
if (isset($_GET['id_bobot'])) {
  $colname_rsBobotUpdate = $_GET['id_bobot'];
}

$query_rsBobotUpdate = sprintf("SELECT * FROM bobot WHERE id_bobot = %s", GetSQLValueString($colname_rsBobotUpdate, "int"));
$rsBobotUpdate = mysqli_query($koneksi, $query_rsBobotUpdate) or die(mysqli_error());
$row_rsBobotUpdate = mysqli_fetch_assoc($rsBobotUpdate);
$totalRows_rsBobotUpdate = mysqli_num_rows($rsBobotUpdate);
?>
<!DOCTYPE html>
...
<div class="w3-container w3-light-gray">
  ...
  <li style="border-bottom:1px solid #DDD;"><i class="fa fa-times fa-fw"></i> Keluar</li>
                    ...
                    <div class="w3-col l9" style="padding-left:8px;">
                    	<div class="w3-border w3-padding"><!-- InstanceBeginEditable name="EditRegion2" --><div class="w3-border-bottom" style="padding-bottom:10px; margin-top:8px;"><strong><i class="fa fa-edit"></i> UBAH DATA RULE & HIMPUNAN FUZZY</strong></div>
                        
<form method="post" name="form1" action="<?php echo $editFormAction; ?>">
<div style="margin-top:8px;">
<label>Ubah Kode Penyakit</label>
<select name="kd_penyakit" class="w3-input w3-border w3-small" required style="cursor:pointer">
  ...
  if($datapenyakit === FALSE){
   ...
   ?>
        <option value="<?php echo $pkode ?>" <?php if (!(strcmp("$pkode", htmlentities($row_rsBobotUpdate['kd_penyakit'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>><?php echo $pkode." | ".$pnamapenyakit ?></option>
...
<select name="kd_gejala" class="w3-input w3-border w3-small" required style="cursor:pointer">
  ...
  if($datagejala === FALSE){
   ...
   ?>
        <option value="<?php echo $vkode ?>" <?php if (!(strcmp("$vkode", htmlentities($row_rsBobotUpdate['kd_gejala'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>><?php echo $vkode." | ".$vgejala ?></option>
...
<select name="bobot" class="w3-input w3-border w3-small" required style="cursor:pointer">
  ...
  if($dataindikator === FALSE){
   ...
   ?>
        <option value="<?php echo $vnilai ?>" <?php if (!(strcmp("$vnilai", htmlentities($row_rsBobotUpdate['bobot'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>><?php echo $vnilai." | ".$vnamain ?></option>
  ...
  <table align="center" style="display:none">
    <tr valign="baseline">
      ...
      <td><?php echo $row_rsBobotUpdate['id_bobot']; ?></td>
    </tr>
  ...
  <input type="hidden" name="id_bobot" value="<?php echo $row_rsBobotUpdate['id_bobot']; ?>">
</form>
<br>
<?php
mysqli_free_result($rsBobotUpdate);
?>
